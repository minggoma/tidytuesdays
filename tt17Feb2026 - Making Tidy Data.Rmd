...
```{r}
#Goats had an interesting bump in 1980 - did this match the farm area increase

g <- ggplot(data = wide2 %>% select(year_ended_june, 
                                    `Total goats`,
                                    `Total Area of Farms`) %>%
                filter(year_ended_june >= 1980) %>%
                pivot_longer(cols = c(`Total goats`, `Total Area of Farms`),
                             names_to = "vars",
                             values_to = "vals"),
            aes(x = year_ended_june, y = vals)) + geom_point() + facet_wrap(~vars)
